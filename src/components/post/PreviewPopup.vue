...
import PostPreview from '@/components/post/reader/PostPreview.vue';
...
const post = computed(() => draftStore.$state.drafts[draftStore.$state.activeIndex]);
...
onMounted(async () => {
	if (post.value.featuredPhotoCID) {
		await getPhotoFromIPFS(post.value.featuredPhotoCID).then((p) => {
	...
	if (caption?.offsetHeight !== undefined) {
		captionHeight.value = caption.offsetHeight;
	}
			...
			>
				...
				<div class="w-full lg:w-760 lg:max-w-760 h-fit">
					...
					<section class="mb-5 px-5 lg:px-0 pb-16 pt-2 md:pb-5">
						...
						<article v-if="post.featuredPhotoCID !== null" class="relative mb-5 mt-5 flex flex-col justify-end">
							<div
						...
						<article class="mt-5">
							<div class="text-lightPrimaryText dark:text-darkSecondaryText editable content max-w-none break-words">
								<PostPreview
									...
									:encrypted="false"
									:post-image-keys="post.postImages"
								/>
							</div>
						</article>
						<!-- Tags -->
